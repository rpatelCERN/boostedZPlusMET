...
fsig=TFile("BoostedZISRCorrectionNew.root", "READ");
...
fisr=TFile("ISRSystematics.root", "READ");
fstat=TFile("MCStat.root","READ");
fJECup=TFile("JECUp.root", "READ");
fJECdown=TFile("JECDown.root", "READ");
fJERup=TFile("JERUp.root", "READ");
fJERdown=TFile("JERDown.root", "READ");
fSmear=TFile("SmearSysRevised.root", "READ");
count=0
for m in mGo:		
        signal=fsig.Get("T5ZZ%dMETShapeSRMETBins" %m)
	signal.Scale(1.0-ContaminScales[count])
        ISR=fisr.Get("T5ZZ%disrsys" %m)
        MCStat=fstat.Get("T5ZZ%dMCStat" %m)
	JECup=fJECup.Get("T5ZZ%dJEC" %m)
	JECdown=fJECdown.Get("T5ZZ%dJEC" %m)
	JERup=fJERup.Get("T5ZZ%dJER" %m)
	JERdown=fJERdown.Get("T5ZZ%dJER" %m)
	...
	for line in f:
		...
		if "ISRSys" in line:line="ISRSys lnN %g - %g - %g - %g - %g - %g -\n" %(ISR.GetBinContent(1), ISR.GetBinContent(2),ISR.GetBinContent(3),ISR.GetBinContent(4),ISR.GetBinContent(5),ISR.GetBinContent(6))
